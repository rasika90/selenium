...
if (!this.formatterType) {  // this.formatterType is defined for the new Formatter system
  ...
  var subScriptLoader = Components.classes["@mozilla.org/moz/jssubscript-loader;1"].getService(
...
function formatSuite(testSuite, filename) {
  ...
  var formattedSuite = '<!DOCTYPE suite SYSTEM "http://testng.org/testng-1.0.dtd" >\n\n'
                       + '<!--testng.xml-->\n'
                       + '<!--For more details please visit: http://testng.org/doc/documentation-main.html-->\n'
                       + '<suite name="Suite' + suiteClass + '" verbose="1" >\n'
                       + indents(1) + '<test name="' + suiteClass + '">\n'
                       + indents(2) + '<classes>\n'
...
WDAPI.Driver.prototype.findElement = function (locatorType, locator) {
  return new WDAPI.Element(this.ref + ".findElement(" + WDAPI.Driver.searchContext(locatorType,
                                                                                   locator) + ")");
};

WDAPI.Driver.prototype.findElements = function (locatorType, locator) {
  return new WDAPI.ElementList(this.ref + ".findElements(" + WDAPI.Driver.searchContext(locatorType,
                                                                                        locator)
                               + ")");
};
